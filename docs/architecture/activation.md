...
| `declaredKeys` | `declaredKeys(source, dest, format)` | Enforces the declared TOML, JSON, or JSONC keys into an app-owned config file (jsonc writes preserve the host file's comments), preserving the keys the application writes at runtime; equality is structural, so the app's own rewrites never re-trigger it, and the destination is declared preserved so the deploy phase cannot erase it |
