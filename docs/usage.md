...
`w-td` fetches with `--prune`, removes every worktree whose branch tracked a remote branch that no longer exists, deletes those branches, and fast-forwards the main worktree's default branch. It never checks out, and it declines the fast-forward rather than failing when the main worktree is on another branch, is dirty, or carries unpushed commits. Anything it will not remove is named with its reason: locked, prunable, dirty, the current directory, or an upstream that was a local branch rather than a remote one.
