...
1. Deploy — `deployRole()` writes every embedded asset for the selected roles into `~/.config/mev/roles/{role}/`. Skips if already present unless `overwrite` is set, in which case replacement is built in a sibling staging directory before the old role is moved aside and removed.
