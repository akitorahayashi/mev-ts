...
The hidden `mev internal document markdown-to-pdf` and `pdf-to-markdown` commands back the `md2pdf` and `pdf2md` shell aliases. The `document` provisioning target owns Pandoc, Poppler, and Google Chrome; the shell target owns only the aliases.
