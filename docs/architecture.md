...
The `pipx` and `pnpm` manifests share the same removal vocabulary: an optional root `uninstall` list, absent by default, whose names alone are ever removed. A listed name still present in the tool's inventory is uninstalled; an already absent name reports unchanged, so repeat runs stay idempotent. Removals run before installs, and nothing is derived from inventory diffs against the declared items, so per-machine manual installs are never touched.
...
Every `binaries.yml` release manifest has a `binaries.lock.yml` sibling recording the SHA-256 digest of each declared binary's release asset per architecture, generated by `bun run lock` (`scripts/lock-releases.ts`). The script downloads only assets whose exact name/repo/tag is not already locked, so editing one entry re-locks just that entry. Installation verifies the downloaded bytes against the lock before the atomic replace and refuses a mismatch, replacing trust in whatever the remote serves with a reviewed digest; `installedMatches` compares the installed file's digest (repairing a stripped execute bit in place) so idempotency is byte-exact rather than version-string based. A lock entry resolves only on an exact name/repo/tag match, so a retagged release never inherits a stale digest. Preflight validates that the lock fully covers the manifest for both architectures, failing `check` when a manifest edit lacks a lock refresh. The lock is an embedded role asset, so a digest change flips the target signature and `sync` reselects the target.
